	...
	int isBackground)	/* To wait for program or add to the list */
{
	...
	if (!childPid) {
		...
		if (isBackground){
			...
			if (childExitStatus!=0) { fprintf(stderr, "Error with code %d from %s\n",childExitStatus,program); }
			gettimeofday(&endTime, NULL);
			printf("Foreground process %d executed in  %ld ms\n",childPid, (endTime.tv_sec - startTime.tv_sec)*1000 + (endTime.tv_usec - startTime.tv_usec)/1000);
		}
	}
	return 0;
}
